(function($) {
    ...
    // -------------------------------------------------------------------------
    // Forms (service, availability, payments, design, settings)
    // -------------------------------------------------------------------------
    function initForms() {
    ...
    function saveService(form) {
        ...
        var endpoint = serviceId
            ? restUrl + 'admin/services/' + serviceId
            : restUrl + 'admin/services';
    ...
    function saveAvailability(form) {
        ...
        apiRequest('POST', restUrl + 'admin/availability', payload)
            .done(function(res) {
                if (res.success) {
            ...
            .fail(function(xhr) { showRequestError(getRequestErrorMessage(xhr)); });
    }
    ...
    function generateAvailabilityPreview() {
        ...
        apiRequest('POST', restUrl + 'admin/availability/preview', payload)
            .done(function(res) {
        ...
        apiRequest('POST', restUrl + 'admin/availability/validate', validatePayload)
            .done(function(res) {
    ...
    function validateBeforeSave() {
        ...
        apiRequest('POST', restUrl + 'admin/availability/validate', payload)
            .done(function(res) {
                var conflicts = res.conflicts || [];
                var warnings = res.warnings || [];
                if (!conflicts.length && !warnings.length) {
                    ...
                    if (conflicts.length) {
                        ...
                        conflicts.forEach(function(c) { html += '<li>' + escapeHtml(c.message) + '</li>'; });
                        html += '</ul></div>';
                    }
                    if (warnings.length) {
                        html += '<div class="notice notice-warning"><p><strong>' + warnings.length + ' advertencia(s):</strong></p><ul>';
                        warnings.forEach(function(w) { html += '<li>' + escapeHtml(w.message) + '</li>'; });
                        html += '</ul></div>';
                    }
                    $result.html(html);
                }
    ...
    function buildAvailabilityPayload() {
        ...
        $('.obwp-exception-row:not(.obwp-template)').each(function() {
            ...
            if (date) {
                payload.blocks.push({ start_at: date + ' 00:00:00', end_at: date + ' 23:59:59', reason: reason });
            }
        });
    ...
    function loadSnapshotsForCurrentScope() {
        ...
        apiRequest('GET', restUrl + 'admin/availability/snapshots?' + params)
            .done(function(res) {
    ...
    function restoreSnapshot(id) {
        apiRequest('POST', restUrl + 'admin/availability/snapshots/' + id + '/restore', {})
            .done(function(res) {
                if (res.success) {
    ...
    function deleteSnapshot(id) {
        apiRequest('DELETE', restUrl + 'admin/availability/snapshots/' + id, {})
            .done(function(res) {
                if (res.success) {
    ...
    function loadAvailabilityForCurrentScope() {
        var scopeType = $('#scope_type').val() || 'global';
        var scopeId = scopeType === 'service'
            ? (parseInt($('#scope_service_id').val(), 10) || 0)
            : (scopeType === 'resource' ? (parseInt($('#scope_resource_id').val(), 10) || 0) : 0);
        ...
        apiRequest('GET', restUrl + 'admin/availability?scope_type=' + encodeURIComponent(scopeType) + '&scope_id=' + scopeId)
            .done(function(res) {
    ...
    function hydrateAvailabilityForm(rules, blocks) {
        ...
        rules.forEach(function(rule) {
            ...
            if (rule.rule_type === 'break' && rule.weekday) {
                var breakHtml = '<div class="obwp-break-row" data-day="' + rule.weekday + '">' +
                    '<input type="time" class="break-from" value="' + escapeHtml((rule.time_from || '').slice(0, 5)) + '"> <span>a</span> ' +
                    '<input type="time" class="break-to" value="' + escapeHtml((rule.time_to || '').slice(0, 5)) + '"> ' +
                    '<button type="button" class="ob-btn ob-btn-danger ob-btn-sm obwp-remove-break">Eliminar</button>' +
                    '</div>';
    ...
    function savePayments(form) {
        ...
        apiRequest('POST', restUrl + 'admin/settings/payments', payload)
            .done(function(res) {
                if (res.success) {
                    showNotice('Guardado correctamente.', 'success');
                } else {
    ...
    function saveDesign(form) {
        ...
        // Reconstruct radius with px
        if (payload.radius) {
            payload.radius = payload.radius + 'px';
        }

        apiRequest('POST', restUrl + 'admin/settings/design', payload)
            .done(function(res) {
                if (res.success) {
                    showNotice('Guardado correctamente.', 'success');
                    if (res.contrast_warnings && res.contrast_warnings.length) {
                        ...
                        res.contrast_warnings.forEach(function(w) { warnHtml += escapeHtml(w) + '<br>'; });
                        warnHtml += '</div></div>';
    ...
    function saveSettings(form) {
        ...
        apiRequest('POST', restUrl + 'admin/settings', payload)
            .done(function(res) {
                if (res.success) {
                    showNotice('Guardado correctamente.', 'success');
                } else {
    ...
    // -------------------------------------------------------------------------
    // Delete buttons (services, resources)
    // -------------------------------------------------------------------------
    function initDeleteButtons() {
        $(document).on('click', '.obwp-delete-service', function() {
            ...
            apiRequest('DELETE', restUrl + 'admin/services/' + id)
                .done(function(res) {
                    if (res.success) {
        ...
        $(document).on('click', '.obwp-delete-resource', function() {
            ...
            apiRequest('DELETE', restUrl + 'admin/resources/' + id)
                .done(function(res) {
                    if (res.success) {
    ...
    function initAgenda() {
        ...
        apiRequest('GET', restUrl + 'admin/services')
            .done(function(res) {
        ...
        $(document).on('click', '#obwp-detail-reschedule', function() {
            ...
            if (!newDateTime || !/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}$/.test(newDateTime.trim())) {
                if (newDateTime) showRequestError('Formato inválido. Use YYYY-MM-DD HH:MM.');
                return;
            }
            apiRequest('PATCH', restUrl + 'admin/bookings/' + id, { action: 'reschedule', start_at: newDateTime.trim() })
                .done(function(res) {
                    if (res.success) {
                        loadAgenda();
                        loadBookingDetail(id);
        ...
        // Render navigation toolbar and load initial data
        renderAgendaNav();
        loadAgenda();
    }
    ...
    function loadAgenda() {
        ...
        apiRequest('GET', restUrl + 'admin/bookings?' + $.param(params))
            .done(function(res) {
    ...
    function renderAgenda(bookings, range) {
        ...
        if (!bookings.length) {
            html += '<p class="obwp-empty">' + (strings.no_bookings || 'No hay reservas para este período.') + '</p>';
            $main.html(html);
            return;
        }
    ...
    function loadBookingDetail(id) {
        ...
        apiRequest('GET', restUrl + 'admin/bookings/' + id)
            .done(function(res) {
                var b = res.booking;
                if (!b) { $sidebar.html('<p class="obwp-empty">No encontrada.</p>'); return; }
                ...
                if (b.status === 'pending' || b.status === 'confirmed') {
                    ...
                    html += '<button class="ob-btn ob-btn-secondary ob-btn-sm" id="obwp-detail-noshow" data-id="' + escapeHtml(b.id) + '">' + (strings.no_show || 'No asistió') + '</button> ';
    ...
    function bookingAction(id, action) {
        apiRequest('PATCH', restUrl + 'admin/bookings/' + id, { action: action })
            .done(function(res) {
                if (res.success) {
                    loadAgenda();
                    loadBookingDetail(id);
                    showNotice('Acción realizada correctamente.', 'success');
                } else {
                    showRequestError(res.error || strings.error_generic);
                }
            })
    ...
    function updateDesignPreview() {
        ...
        var previewHtml = '<div class="obwp-preview-inner" style="' + cssVars + 'background:' + bg + ';color:' + text + ';font-family:' + font + ';padding:24px">' +
            stepperHtml +
    ...
    function buildStepperHtml(primary, state) {
        ...
        for (var i = 0; i < _stepLabels.length; i++) {
            ...
            html += '<div style="display:flex;flex-direction:column;align-items:center;gap:4px">' +
                '<div style="width:28px;height:28px;border-radius:50%;background:' + numBg + ';color:' + numColor + ';display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700">' + (i + 1) + '</div>' +
                '<span style="font-size:11px;color:' + labelColor + ';white-space:nowrap;font-weight:' + (isActive ? '700' : '400') + '">' + _stepLabels[i] + '</span>' +
                '</div>';
    ...
    function buildPreviewBody(primary, bg, text, font, radius, state) {
        ...
        if (state === 'service') {
            return '<p style="font-size:13px;color:#888;margin:0 0 12px">Elige un servicio para comenzar</p>' +
                '<div style="' + card + 'display:flex;justify-content:space-between;align-items:center;cursor:pointer">' +
                    '<div><p style="font-weight:700;font-size:15px;margin:0 0 4px">Consulta — 60 min</p>' +
                    '<p style="margin:0;font-size:13px;color:#888">Presencial · $35.000</p></div>' +
                    '<button style="background:' + primary + ';color:#fff;border:none;border-radius:' + radius + ';padding:8px 18px;font-family:' + font + ';font-size:13px;cursor:pointer">Elegir</button>' +
                '</div>' +
                '<div style="' + card + 'margin-top:10px;opacity:0.55">' +
                    '<p style="font-weight:700;font-size:15px;margin:0 0 4px">Seguimiento — 30 min</p>' +
                    '<p style="margin:0;font-size:13px;color:#888">Online · $18.000</p>' +
                '</div>';
        }
        if (state === 'calendar') {
            ...
            calHtml += '</div>';
            // 3 weeks only: days 1–21, starting on Wednesday (2 empty slots)
            ...
            cells.forEach(function(n) {
                if (!n) { calHtml += '<span style="' + W + '"></span>'; return; }
                var isSel   = n === 15;
        ...
        if (state === 'form') {
            var field = function(label, placeholder) {
                return '<div style="margin-bottom:14px">' +
                    '<label style="display:block;font-size:12px;font-weight:600;margin-bottom:5px;color:#555">' + label + '</label>' +
                    '<input style="width:100%;border:1px solid #ddd;border-radius:' + radius + ';padding:9px 12px;font-family:' + font + ';font-size:14px;color:' + text + ';background:' + bg + ';box-sizing:border-box" placeholder="' + placeholder + '" readonly>' +
                    '</div>';
            };
            return field('Nombre', 'Tu nombre') + field('Correo electrónico', 'correo@ejemplo.com') + field('Teléfono', '+56 9 1234 5678') +
                '<div style="margin-top:6px">' +
                '<button style="width:100%;background:' + primary + ';color:#fff;border:none;border-radius:' + radius + ';padding:12px;font-family:' + font + ';font-size:15px;font-weight:700;cursor:pointer">Confirmar reserva</button>' +
                '</div>';
        }
        // Confirmation states
        var stateMeta = {
        ...
        var rows = [
            ...
            ['Fecha y hora', 'Mié 15 may · 11:00'],
            ['Profesional', 'Dr. García'],
            ['Referencia', '#OB-2024-0042']
        ];
        var rowsHtml = rows.map(function(r) {
            return '<div style="display:flex;justify-content:space-between;align-items:baseline;padding:12px 0;border-bottom:1px solid #f3f3f3">' +
                '<span style="font-size:13px;color:#888">' + r[0] + '</span>' +
                '<span style="font-size:14px;font-weight:700;color:' + text + '">' + r[1] + '</span>' +
                '</div>';
        }).join('');
        return '<div style="border-radius:' + radius + ';overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.08)">' +
            '<div style="background:' + meta.bg + ';color:#fff;padding:24px 20px">' +
                '<div style="font-size:28px;margin-bottom:8px">' + meta.icon + '</div>' +
                '<div style="font-size:18px;font-weight:700">' + meta.label + '</div>' +
                (state === 'pending' ? '<div style="font-size:13px;opacity:0.85;margin-top:4px">Tu pago está siendo verificado. Te enviaremos un correo.</div>' : '') +
                (state === 'failed'  ? '<div style="font-size:13px;opacity:0.85;margin-top:4px">No se realizó ningún cobro. Puedes intentar de nuevo.</div>' : '') +
            '</div>' +
            '<div style="background:#fff;padding:4px 20px 8px">' + rowsHtml + '</div>' +
            '<div style="background:#fafafa;border-top:1px solid #eee;padding:14px 20px;display:flex;justify-content:center;gap:12px">' +
                (state !== 'failed' ? '<button style="padding:9px 20px;border-radius:' + radius + ';border:1px solid #ddd;background:#fff;font-family:' + font + ';font-size:13px;cursor:pointer">Reprogramar</button>' : '') +
                (state === 'failed'  ? '<button style="padding:9px 20px;border-radius:' + radius + ';border:none;background:' + primary + ';color:#fff;font-family:' + font + ';font-size:13px;cursor:pointer">Volver a intentar</button>' : '') +
                (state !== 'failed' ? '<button style="padding:9px 20px;border-radius:' + radius + ';border:1px solid #ddd;background:#fff;font-family:' + font + ';font-size:13px;cursor:pointer;color:#c0392b">Cancelar</button>' : '') +
            '</div>' +
            '</div>';
    }
    ...
    function showLiveContrastWarning(primary, bg, text) {
        ...
        if (warnings.length) {
            ...
            warnings.forEach(function(w) { html += escapeHtml(w) + '<br>'; });
            html += '</div></div>';
            if ($existing.length) { $existing.replaceWith(html); } else { $('#obwp-design-form .obwp-tabs').before(html); }
        } else {
            $existing.remove();
        }
    }
    ...
    function loadFormFieldsPreview() {
        apiRequest('GET', restUrl + 'admin/form-fields')
            .done(function(res) {
        ...
        $(document).on('click', '#obwp-save-form-fields', function() {
            ...
            apiRequest('POST', restUrl + 'admin/form-fields', { fields: fields })
                .done(function(res) {
                    if (res.success) {
    ...
    function initEmailTemplates() {
        ...
        apiRequest('GET', restUrl + 'admin/email-templates')
            .done(function(res) {
        ...
        $(document).on('click', '.obwp-save-template', function() {
            ...
            apiRequest('POST', restUrl + 'admin/email-templates/' + key, { subject: subject, body: body })
                .done(function(res) {
        ...
        $(document).on('click', '#obwp-send-test-email', function() {
            ...
            if (!to) { showRequestError('Ingresa un email.'); return; }
            $(this).prop('disabled', true).text('Enviando...');
            apiRequest('POST', restUrl + 'admin/email-test', { to: to })
                .done(function(res) {
                    if (res.success) showNotice(res.message || 'Email enviado.', 'success');
                    else showRequestError(res.error || 'Error al enviar.');
                })
                .fail(function(xhr) { showRequestError(getRequestErrorMessage(xhr)); })
                .always(function() { $('#obwp-send-test-email').prop('disabled', false).text('Enviar prueba'); });
        });
    }

    // -------------------------------------------------------------------------
    // Notification settings (tab Notificaciones)
    // -------------------------------------------------------------------------
    function initNotificationSettings() {
        ...
        function loadNotifSettings() {
            apiRequest('GET', restUrl + 'admin/settings/notifications')
                .done(function(res) {
                    // Email
        ...
        function loadSmsTemplates() {
            ...
            if (!$container.length) return;
            apiRequest('GET', restUrl + 'admin/sms-templates')
                .done(function(res) {
                    var templates = res.templates || {};
                    var labels = {
                        booking_confirmed:   'Reserva confirmada',
        ...
        function loadWaTemplates() {
            ...
            if (!$container.length) return;
            apiRequest('GET', restUrl + 'admin/whatsapp-templates')
                .done(function(res) {
                    var templates = res.templates || {};
                    var labels = {
                        booking_confirmed:   'Reserva confirmada',
        ...
        function loadNotifLog() {
            ...
            apiRequest('GET', restUrl + 'admin/notification-logs?' + params)
                .done(function(res) {
                    var logs = res.logs || [];
                    if (!logs.length) {
        ...
        $(document).on('click', '#obwp-save-email-notif', function() {
            ...
            apiRequest('POST', restUrl + 'admin/settings/notifications', data)
                .done(function(res) {
                    if (res.success) showNotice('Ajustes de email guardados.', 'success');
                    else showRequestError(res.error || strings.error_generic);
                })
                .fail(function(xhr) { showRequestError(getRequestErrorMessage(xhr)); })
                .always(function() { $btn.prop('disabled', false).text('Guardar ajustes de email'); });
        });
        ...
        $(document).on('click', '#obwp-save-wa-notif', function() {
            ...
            apiRequest('POST', restUrl + 'admin/settings/notifications', data)
                ...
                .fail(function(xhr) { showRequestError(getRequestErrorMessage(xhr)); })
                .always(function() { $btn.prop('disabled', false).text('Guardar ajustes de WhatsApp'); });
        });

        // save individual WhatsApp template
        $(document).on('click', '.obwp-save-wa-template', function() {
            var $block = $(this).closest('.obwp-wa-template');
            var key  = $block.data('key');
            var body = $block.find('.wt-body').val();
            var $btn = $(this).prop('disabled', true).text('Guardando...');
            apiRequest('POST', restUrl + 'admin/whatsapp-templates/' + key, { body: body })
                .done(function(res) {
        ...
        $(document).on('click', '#obwp-send-test-wa', function() {
            ...
            if (!to) { showRequestError('Ingresa un número de destino.'); return; }
            var $btn = $(this).prop('disabled', true).text('Enviando...');
            apiRequest('POST', restUrl + 'admin/whatsapp-test', { to: to })
                .done(function(res) {
                ...
                .fail(function(xhr) { showRequestError(getRequestErrorMessage(xhr)); })
                .always(function() { $btn.prop('disabled', false).text('Enviar prueba'); });
        });
        ...
        $(document).on('click', '#obwp-save-sms-notif', function() {
            ...
            apiRequest('POST', restUrl + 'admin/settings/notifications', data)
                ...
                .fail(function(xhr) { showRequestError(getRequestErrorMessage(xhr)); })
                .always(function() { $btn.prop('disabled', false).text('Guardar ajustes de SMS'); });
        });

        // save individual SMS template
        $(document).on('click', '.obwp-save-sms-template', function() {
            var $block = $(this).closest('.obwp-wa-template');
            var key  = $block.data('key');
            var body = $block.find('.wt-body').val();
            var $btn = $(this).prop('disabled', true).text('Guardando...');
            apiRequest('POST', restUrl + 'admin/sms-templates/' + key, { body: body })
                .done(function(res) {
                    if (res.success) showNotice('Mensaje guardado.', 'success');
                    else showRequestError(res.error || strings.error_generic);
                })
        ...
        $(document).on('click', '#obwp-send-test-sms', function() {
            ...
            if (!to) { showRequestError('Ingresa un número de destino.'); return; }
            var $btn = $(this).prop('disabled', true).text('Enviando...');
            apiRequest('POST', restUrl + 'admin/sms-test', { to: to })
                .done(function(res) {
        ...
        // load notification log on button click or auto on init
        $(document).on('click', '#obwp-notif-log-load', function() {
            loadNotifLog();
        });

        // auto-load on init if the table container is present
    ...
    function initPaymentsPage() {
        ...
        if ($gateways.length) {
            apiRequest('GET', restUrl + 'admin/gateways')
                .done(function(res) {
                    var gateways = res.gateways || [];
                    if (!gateways.length) {
                        $gateways.html('<p>No hay medios de pago disponibles para este país.</p>');
                        return;
                    }
                    var html = '';
                    gateways.forEach(function(g) {
                        var status = g.health && g.health.status ? g.health.status : (g.configured ? 'ok' : 'warning');
                        var note = g.configured ? 'Configurado' : 'Configuración incompleta';
        ...
        if ($recent.length) {
            apiRequest('GET', restUrl + 'admin/payments?limit=20')
                .done(function(res) {
        ...
        $(document).on('click', '.obwp-refund-btn', function() {
            ...
            var id = $(this).data('id');
            var $btn = $(this);
            apiRequest('POST', restUrl + 'admin/payments/' + id + '/refund', {})
                .done(function(res) {
                    if (res.success) {
        ...
        $(document).on('click', '.obwp-change-payment-status', function() {
            ...
            if (allowedStatuses.indexOf(status.trim().toLowerCase()) === -1) {
                showRequestError('Estado no válido. Use: ' + allowedStatuses.join(', '));
                return;
            }
            ...
            apiRequest('POST', restUrl + 'admin/payments/' + id + '/status', { status: status, reason: reason })
                .done(function(res) {
                    if (res.success) {
    ...
    function loadAuditLogs(offset) {
        ...
        apiRequest('GET', restUrl + 'admin/audit-logs?' + $.param(query))
            .done(function(res) {
    ...
    function loadAuditLogDetail(id) {
        ...
        apiRequest('GET', restUrl + 'admin/audit-logs/' + id)
            .done(function(res) {
    ...
    function syncAuditFiltersToUrl(query) {
        ...
        Object.keys(query || {}).forEach(function(key) {
            if (query[key] !== '' && query[key] !== null && query[key] !== undefined && key !== 'limit') {
    ...
    function loadEntityAuditLogs(entityType, entityId, target) {
        ...
        apiRequest('GET', restUrl + 'admin/audit-logs?' + $.param({ entity_type: entityType, entity_id: entityId, limit: 10 }))
            .done(function(res) {
                var logs = res.logs || [];
                if (!logs.length) {
    ...
    // -------------------------------------------------------------------------
    // Gateway settings (Stripe, MercadoPago credentials)
    // -------------------------------------------------------------------------
    function initGatewaySettings() {
        $(document).on('submit', '.obwp-gateway-settings-form', function(e) {
            ...
            apiRequest('POST', restUrl + 'admin/settings/gateway/' + key, data)
                .done(function(res) {
    ...
    // -------------------------------------------------------------------------
    // Form fields editor (Diseño > Campos tab)
    // -------------------------------------------------------------------------
    function initFormFieldsEditor() {
        // Handled inside initDesign() > loadFormFieldsPreview()
    }
    ...
    function apiRequest(method, url, data) {
        ...
        return $.ajax(opts).fail(function(xhr) {
            if (xhr && (xhr.status === 401 || xhr.status === 403)) {
                showNotice(strings.session_expired || 'Tu sesión expiró. Recarga la página.', 'error');
            }
        });
    }
    ...
    function getRequestErrorMessage(xhr) {
        if (xhr && (xhr.status === 401 || xhr.status === 403)) {
            return strings.session_expired || 'Tu sesión expiró. Recarga la página.';
        }
    ...
    function initDashboardData() {
        ...
        apiRequest('GET', restUrl + 'admin/dashboard').done(function(res) {
            ...
            function renderBookingList(bookings) {
                ...
                var html = '<ul class="obwp-booking-list">';
                bookings.forEach(function(b) {
                    ...
                    html += '<li>';
                    html += '<strong>' + escapeHtml(label.trim()) + '</strong> &mdash; ';
                    html += escapeHtml(b.service_name || 'Servicio') + ' &mdash; ';
                    html += escapeHtml(name.trim());
                    html += ' <span class="obwp-status obwp-status--' + escapeHtml(b.status) + '">' + escapeHtml(b.status) + '</span>';
                    html += '</li>';
                });
                html += '</ul>';
            ...
            // Show today's bookings; if none, fall through to upcoming
            ...
            if (todayBookings.length) {
                ...
                html += '<p class="obwp-note" style="margin:0 0 8px"><strong>Próximas reservas</strong></p>';
                html += renderBookingList(upcomingBookings);
            } else {
                html += '<p class="obwp-empty">No hay reservas próximas.</p><a href="' + escapeHtml(adminUrl('admin.php?page=openbooking-agenda')) + '" class="ob-btn ob-btn-secondary">Ver agenda</a>';
            }
            $bookingContainer.html(html);

            // Recent bookings widget (nuevas reservas)
            var $recent = $('#obwp-recent-bookings');
            if ($recent.length) {
                if (recentBookings.length) {
                    $recent.html(renderBookingList(recentBookings));
                } else {
                    $recent.html('<p class="obwp-empty">Sin reservas nuevas en las últimas 48 h.</p>');
                }
            }
            ...
            if (attentionRequired.length) {
                uHtml = attentionRequired.map(function(b) {
                    ...
                    var name = (b.first_name || '') + ' ' + (b.last_name || '');
                    if (name.trim() === '') name = b.customer_name || b.email || '';
                    var reason = b.urgency_reason === 'payment_window_expired'
                        ? 'Pago vencido'
                        : (b.urgency_reason === 'starting_soon' ? 'Comienza pronto' : 'Revisar');
                    return '<li class="ob-urgency-item">' +
                        '<span class="dashicons dashicons-warning"></span>' +
                        '<span><strong>' + escapeHtml(label.trim()) + '</strong> &mdash; ' +
                        escapeHtml(b.service_name || 'Servicio') + ' &mdash; ' +
                        escapeHtml(name.trim()) + ' <em>(' + escapeHtml(reason) + ')</em></span>' +
                        '</li>';
                }).join('');
                ...
                uHtml = urgency.map(function(u) {
                    return '<li class="ob-urgency-item">' +
                        '<span class="dashicons dashicons-warning"></span>' +
                        '<span>' + escapeHtml(u.label) + ' (<strong>' + u.count + '</strong>)</span>' +
                        '</li>';
                }).join('');
            }
            ...
            $bookingContainer.html('<p class="obwp-empty">Error al cargar reservas. Recarga la página.</p>');
        });
        ...
        $(document).on('click', '#obwp-run-integrity', function() {
            ...
            refreshIntegritySummary(true).always(function() {
                $btn.prop('disabled', false).text('Ver diagnóstico');
            });
        });
    }
    ...
    function refreshIntegritySummary(showDetails) {
        ...
        apiRequest('GET', restUrl + 'admin/integrity-check').done(function(res) {
            ...
            var summaryText = errCount > 0
                ? 'Diagnóstico: ' + errCount + ' error(es) y ' + warnCount + ' advertencia(s).'
                : (warnCount > 0
                    ? 'Diagnóstico: ' + warnCount + ' advertencia(s).'
            ...
            if (showDetails || warnCount > 0 || errCount > 0) {
                ...
                html += '<p><strong>' + escapeHtml(res.message || 'Resultado de diagnóstico') + '</strong></p>';
            ...
            if ($summary.length) {
                $summary.html('<strong>Diagnóstico no disponible</strong> <span class="ob-meta">' + escapeHtml(getRequestErrorMessage(xhr)) + '</span>');
            }
            if ($result.length) {
