(() => {
  // resources/assets/public/booking/legacy-booking-flow.js
  (function($) {
    "use strict";
    ...
    var State = {
      ...
      loadedAt: Math.floor(Date.now() / 1e3),
    ...
    var Api = {
      ...
      errorMsg: function(xhr, fallback) {
        fallback = fallback || cfg.strings.error_generic || "Ha ocurrido un error. Int\xE9ntalo de nuevo.";
    ...
    var Summary = {
      render: function() {
        ...
        if (State.date || State.time) {
          html += "<p><span>" + escapeHtml(cfg.strings.summary_datetime || "Fecha y hora") + "</span><strong>" + escapeHtml(formatDateHuman(State.date)) + (State.time ? " \xB7 " + escapeHtml(State.time) : "") + "</strong></p>";
        }
        if (cfg.paymentMode === "deposit" && State.servicePriceMinor > 0) {
          ...
          var dueNowStr = (dueNowMinor / 100).toLocaleString(void 0, { minimumFractionDigits: 2, maximumFractionDigits: 2 });
      ...
      getPaymentText: function() {
        ...
        if (cfg.paymentMode === "deposit") {
          return (cfg.strings.pay_now || "Pagas ahora") + " " + cfg.depositPercent + "% \xB7 " + (cfg.strings.pay_later || "Pagas despu\xE9s");
        }
    ...
    var Calendar = {
      ...
      render: function() {
        ...
        Api.get("availability/dates?service_id=" + State.serviceId + "&month=" + month).done(function(res) {
          ...
          var dayNames = ["Lun", "Mar", "Mi\xE9", "Jue", "Vie", "S\xE1b", "Dom"];
          ...
          dayNames.forEach(function(d2) {
            html += '<div class="obwp-calendar-day-header" role="columnheader">' + d2 + "</div>";
          });
      ...
      selectDate: function(date) {
        ...
        Api.track("date_selected", { service_id: State.serviceId, date });
        Session.save();
        TimeSlots.load(date);
      }
    };
    var TimeSlots = {
      ...
      load: function(date, opts) {
        ...
        var version = ++this._requestVersion;
        var self = this;
        Api.get("availability?service_id=" + State.serviceId + "&date=" + date).done(function(res) {
          if (version !== self._requestVersion) return;
          $section.find(".obwp-notice").not(opts.notice ? "" : ".obwp-notice").remove();
          var slots = (res.slots || []).filter(function(s) {
            return s.available_capacity > 0;
          });
          if (!slots.length) {
            $slots.html(
              '<p class="obwp-empty">' + (cfg.strings.no_slots || "No hay horarios disponibles para este d\xEDa.") + '</p><p class="obwp-empty-help">' + escapeHtml(cfg.strings.no_slots_help || "Intenta seleccionar otro d\xEDa en el calendario.") + "</p>"
            );
            self._stopPoll();
            return;
          }
          ...
          slots.forEach(function(slot) {
            ...
            html += '<div class="obwp-time-slot' + selected + '" role="option" tabindex="0" aria-label="' + time + '" aria-selected="' + (State.startAt === slot.start_at ? "true" : "false") + '" data-start="' + slot.start_at + '" data-end="' + slot.end_at + '" data-resource="' + (slot.resource_id || "") + '">' + time + cap + "</div>";
          });
          $slots.html(html);
          self._startPoll(date);
        }).fail(function() {
          $slots.html(
            '<div class="obwp-state obwp-state--failed" role="alert"><p class="obwp-error">' + (cfg.strings.error_slots_load || "No pudimos cargar los horarios.") + '</p><button class="obwp-btn obwp-btn--secondary" id="obwp-retry-slots">' + (cfg.strings.retry || "Volver a intentar") + "</button></div>"
          );
          self._stopPoll();
    ...
    var Form = {
      render: function() {
        ...
        fields.forEach(function(f) {
          ...
          if (f.field_key === "notes") {
            ...
            html += '<input type="' + t + '" id="obwp-' + f.field_key + '" name="' + f.field_key + '" placeholder="' + ph + '" value="' + escapeHtml(State.customerDraft[f.field_key] || "") + '"' + reqAttr + ' autocomplete="' + ac + '">';
          }
          html += "</div>";
        });
        if (cfg.privacyConsentRequired && cfg.privacyPolicyUrl) {
          var policyLink = '<a href="' + escapeHtml(cfg.privacyPolicyUrl) + '" target="_blank" rel="noopener noreferrer">' + escapeHtml(cfg.strings.privacy_policy || "pol\xEDtica de privacidad") + "</a>";
          html += '<div class="obwp-field obwp-field--privacy"><label class="obwp-field-label obwp-field-label--checkbox"><input type="checkbox" id="obwp-privacy-consent" name="privacy_consent" required aria-required="true"> ' + escapeHtml(cfg.strings.privacy_consent_prefix || "He le\xEDdo y acepto la") + " " + policyLink + ' <span aria-hidden="true">*</span></label></div>';
        }
        $form.html(html);
      ...
      _defaultFields: function() {
        return [
          ...
          { field_key: "email", label: "Correo electr\xF3nico", placeholder: "tu@correo.com", is_required: 1, is_enabled: 1 },
          { field_key: "phone", label: "Tel\xE9fono", placeholder: "+56 9 1234 5678", is_required: 0, is_enabled: 1 },
      ...
      validate: function() {
        ...
        if ($emailEl.length && $emailEl.val() && !isValidEmail($emailEl.val())) {
          $emailEl.after('<div class="obwp-error" role="alert">' + (cfg.strings.invalid_email || "Correo no v\xE1lido.") + "</div>");
          valid = false;
        }
        var $phoneEl = $("#obwp-phone");
        if ($phoneEl.length && $phoneEl.val() && !isValidPhone($phoneEl.val())) {
          $phoneEl.after('<div class="obwp-error" role="alert">' + (cfg.strings.invalid_phone || "Ingresa un tel\xE9fono v\xE1lido.") + "</div>");
          valid = false;
        }
        return valid;
      },
      submit: function() {
        ...
        Api.post("bookings", payload).done(function(res) {
          ...
          if (xhr.status === 409) {
            ...
            Steps.show(2);
            Calendar.render();
            TimeSlots.load(State.date, { notice });
            return;
          }
    ...
    var Payment = {
      initiate: function(bookingId, booking) {
        ...
        $panel.html(
          '<div class="obwp-state obwp-state--redirecting" role="status"><p class="obwp-loading">' + escapeHtml(cfg.strings.payment_redirecting || "Redirigiendo al proveedor de pago\u2026") + '</p><p class="obwp-note">' + escapeHtml(cfg.strings.payment_redirect || "Te llevaremos al proveedor de pago para completar la operaci\xF3n.") + "</p></div>"
        );
        ...
        Api.post("payments/create", {
          ...
          Confirm.render(finalBooking, {
            title: res.type === "manual_pending" ? cfg.strings.manual_pending_title || "Tu reserva qued\xF3 registrada" : "",
            subtitle: res.type === "manual_pending" ? cfg.strings.manual_pending_note || "Seleccionaste pago manual. La reserva quedar\xE1 confirmada una vez que validemos tu pago." : res.type === "manual" ? cfg.strings.manual_note || "Pago manual seleccionado. Tu reserva qued\xF3 registrada." : ""
          });
        }).fail(function(xhr) {
      ...
      _renderFailed: function(msg) {
        return '<div class="obwp-state obwp-state--failed" role="alert"><div class="obwp-state-icon" aria-hidden="true">&#9888;</div><h2>' + escapeHtml(cfg.strings.payment_failed || "El pago no se complet\xF3") + "</h2><p>" + escapeHtml(msg) + '</p><button class="obwp-btn obwp-btn--primary" id="obwp-retry-btn">' + escapeHtml(cfg.strings.retry || "Volver a intentar") + "</button></div>";
      },
      ...
      verifyResult: function(paymentResult, token) {
        ...
        Api.get("bookings/public/" + token).done(function(res) {
          ...
          if (booking.payment_status === "paid" && booking.status === "confirmed") {
            ...
            Confirm.render(booking, { subtitle: cfg.strings.payment_success || "Pago procesado correctamente. Tu reserva est\xE1 confirmada." });
            return;
          }
          if (booking.payment_status === "paid" && booking.status !== "confirmed") {
      ...
      _renderReturnState: function(paymentResult, booking) {
        ...
        if (isPending) {
          $('[data-panel="5"]').html(
            '<div class="obwp-state obwp-state--pending" role="status"><div class="obwp-state-icon" aria-hidden="true">&#9202;</div><h2>' + escapeHtml(cfg.strings.payment_pending_title || "Pago en proceso") + "</h2><p>" + escapeHtml(cfg.strings.payment_pending_note || "Tu pago est\xE1 siendo verificado por el proveedor. Esto puede tardar unos minutos.") + '</p><p class="obwp-note">' + escapeHtml(cfg.strings.payment_pending_what_happens || "No se ha cobrado nada hasta que el pago se confirme. Recibir\xE1s un correo con la confirmaci\xF3n.") + '</p><p class="obwp-note">' + escapeHtml(cfg.strings.payment_pending_status || "Estado de la reserva: pendiente de confirmaci\xF3n de pago.") + '</p><button class="obwp-btn obwp-btn--secondary" id="obwp-check-status">' + escapeHtml(cfg.strings.check_status || "Verificar estado") + "</button></div>"
          );
          ...
          $('[data-panel="5"]').html(
            '<div class="obwp-state obwp-state--failed" role="alert"><div class="obwp-state-icon" aria-hidden="true">&#9888;</div><h2>' + escapeHtml(cfg.strings.payment_failed || "El pago no se complet\xF3") + "</h2><p>" + escapeHtml(cfg.strings.payment_no_charge || "No se realiz\xF3 ning\xFAn cobro.") + '</p><p class="obwp-note">' + escapeHtml(cfg.strings.payment_retry_help || "Tu reserva sigue registrada. Puedes intentar pagar de nuevo.") + '</p><button class="obwp-btn obwp-btn--primary" id="obwp-retry-btn">' + escapeHtml(cfg.strings.retry || "Volver a intentar") + "</button></div>"
          );
        }
      },
      _renewPendingHold: function(token, booking) {
        ...
        Api.post("bookings/public/" + token + "/renew-hold", {}).done(function(res) {
    ...
    var Confirm = {
      render: function(booking, options) {
        ...
        var icon = isPendingPayment || isManualPending ? '<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg>' : '<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="20 6 9 17 4 12"></polyline></svg>';
        var title = options.title || (isPendingPayment ? cfg.strings.pending_title || "Tu reserva est\xE1 registrada" : cfg.strings.booking_success || "Tu reserva est\xE1 confirmada");
        var noteText = options.subtitle || (isPendingPayment ? cfg.strings.pending_note || "La reserva se confirmar\xE1 una vez que validemos el pago. Te enviaremos un correo." : "");
        ...
        if (booking.cancel_token) {
          html += '<a href="?obwp_cancel=' + escapeHtml(booking.cancel_token) + '" class="obwp-btn obwp-btn--secondary">' + escapeHtml(cfg.strings.cancel || "Cancelar") + "</a>";
        }
        if (booking.reschedule_token) {
          html += '<a href="?obwp_reschedule=' + escapeHtml(booking.reschedule_token) + '" class="obwp-btn obwp-btn--primary">' + escapeHtml(cfg.strings.reschedule || "Reprogramar") + "</a>";
        }
        html += "</div>";
        html += "</div>";
      ...
      renderError: function(msg) {
        $('[data-panel="5"]').html(
          '<div class="obwp-state obwp-state--failed" role="alert"><p class="obwp-error">' + escapeHtml(msg) + "</p></div>"
        );
      }
    };
    var TokenFlow = {
      ...
      _showCancelModal: function(token) {
        ...
        $app.html(
          '<div class="obwp-token-action"><h2>' + escapeHtml(cfg.strings.cancel_confirm_title || "Cancelar reserva") + "</h2><p>" + escapeHtml(cfg.strings.cancel_confirm || "\xBFEst\xE1s seguro de que deseas cancelar esta reserva? Esta acci\xF3n no se puede deshacer.") + '</p><button class="obwp-btn obwp-btn--primary" id="obwp-confirm-cancel">' + escapeHtml(cfg.strings.yes_cancel || "S\xED, cancelar") + '</button> <a href="' + escapeHtml(window.location.pathname) + '" class="obwp-btn obwp-btn--secondary">' + escapeHtml(cfg.strings.no || "No, volver") + "</a></div>"
        );
        $("#obwp-confirm-cancel").on("click", function() {
          var $btn = $(this).prop("disabled", true).text(cfg.strings.loading || "Procesando...");
          Api.post("bookings/cancel/" + token, {}).done(function(res) {
            if (res.success) {
              $app.html('<div class="obwp-token-action obwp-state obwp-state--success"><div class="obwp-state-icon" aria-hidden="true">&#10003;</div><h2>' + escapeHtml(cfg.strings.cancel_success || "Tu reserva fue cancelada.") + '</h2><p class="obwp-note">' + escapeHtml(cfg.strings.cancel_confirmed_note || "Recibir\xE1s un correo confirmando la cancelaci\xF3n.") + "</p></div>");
            } else {
              $app.html('<div class="obwp-token-action obwp-state obwp-state--failed"><div class="obwp-state-icon" aria-hidden="true">&#9888;</div><h2>' + escapeHtml(cfg.strings.cancel_failed || "No se pudo cancelar") + '</h2><p class="obwp-error">' + escapeHtml(res.error || cfg.strings.error_generic) + '</p><p class="obwp-note">' + escapeHtml(cfg.strings.cancel_help || "Si el enlace expir\xF3, contacta al negocio directamente.") + "</p></div>");
            }
          }).fail(function(xhr) {
            $app.html('<div class="obwp-token-action obwp-state obwp-state--failed"><div class="obwp-state-icon" aria-hidden="true">&#9888;</div><h2>' + escapeHtml(cfg.strings.cancel_failed || "No se pudo cancelar") + '</h2><p class="obwp-error">' + escapeHtml(Api.errorMsg(xhr)) + '</p><p class="obwp-note">' + escapeHtml(cfg.strings.cancel_help || "Si el enlace expir\xF3, contacta al negocio directamente.") + "</p></div>");
          });
        });
      },
      _startReschedule: function(token) {
        ...
        Api.get("bookings/public/" + token).done(function(res) {
          ...
          if (!booking.can_reschedule) {
            $app.html('<div class="obwp-token-action"><p class="obwp-error">' + escapeHtml(cfg.strings.reschedule_unavailable || "Esta reserva ya no se puede reprogramar.") + "</p></div>");
            return;
          }
          ...
          var msg;
          if (xhr.status === 404) {
            msg = cfg.strings.reschedule_token_expired || "Este enlace ya no es v\xE1lido. Es posible que haya expirado o que la reserva haya cambiado de estado.";
      ...
      _renderRescheduleApp: function(booking) {
        ...
        $app.html(
          '<div class="obwp-panel obwp-panel--active" data-panel="reschedule"><h2 class="obwp-panel-title">' + escapeHtml(cfg.strings.reschedule_title || "Reprograma tu reserva") + '</h2><p class="obwp-panel-subtitle">' + escapeHtml(booking.service_name || "") + '</p><p class="obwp-field-label">' + escapeHtml(cfg.strings.current_booking || "Horario actual: ") + escapeHtml(when) + '</p><div class="obwp-date-picker-section" aria-label="Calendario"></div><div class="obwp-time-picker-section" id="obwp-time-section" style="display:none;"><label class="obwp-field-label">' + escapeHtml(cfg.strings.select_time || "Selecciona un horario") + '</label><div id="obwp-time-slots" class="obwp-time-grid" role="list" aria-live="polite"></div></div><div class="obwp-nav-buttons"><button class="obwp-btn obwp-btn--primary" id="obwp-confirm-reschedule">' + escapeHtml(cfg.strings.reschedule_confirm || "Confirmar nueva fecha") + '</button> <a class="obwp-btn obwp-btn--secondary" href="' + escapeHtml(window.location.pathname) + '">' + escapeHtml(cfg.strings.back_home || "Volver") + "</a></div></div>"
        );
      ...
      _submitReschedule: function() {
        ...
        Api.post("bookings/reschedule/" + State.rescheduleToken, {
          ...
          if (res.success && res.booking) {
            ...
            $("#obwp-booking-app").html('<div class="obwp-token-action"><p class="obwp-error">' + escapeHtml(res.error || cfg.strings.error_generic) + "</p></div>");
          }
        }).fail(function(xhr) {
          State.submitting = false;
          if (xhr.status === 409) {
          ...
          var failMsg;
          if (xhr.status === 404) {
            failMsg = cfg.strings.reschedule_failed_expired || "Este enlace ya no es v\xE1lido. La reserva puede haber cambiado de estado.";
          } else if (xhr.status === 403) {
            failMsg = cfg.strings.reschedule_failed_forbidden || "No tienes permiso para realizar esta acci\xF3n.";
          } else {
            failMsg = cfg.strings.reschedule_failed_generic || "No se pudo reprogramar. Por favor intenta de nuevo o contacta al negocio.";
          }
          $("#obwp-booking-app").html('<div class="obwp-token-action"><p class="obwp-error">' + escapeHtml(failMsg) + "</p></div>");
        });
      },
      _showBookingDetails: function(token) {
        ...
        Api.get("bookings/public/" + token).done(function(res) {
          ...
          if (!booking) {
            $app.html(
              '<div class="obwp-token-action obwp-state obwp-state--failed" role="alert"><div class="obwp-state-icon" aria-hidden="true">&#9888;</div><h2>' + escapeHtml(cfg.strings.booking_not_found || "No encontramos esta reserva.") + '</h2><p class="obwp-note">' + escapeHtml(cfg.strings.booking_link_expired || "El enlace puede haber expirado o la reserva fue eliminada.") + "</p></div>"
            );
            return;
          }
          ...
          var icon = isConfirmed ? '<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="20 6 9 17 4 12"></polyline></svg>' : isCancelled ? '<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>' : '<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg>';
          var statusLabel = isConfirmed ? cfg.strings.booking_success || "Tu reserva est\xE1 confirmada" : isCancelled ? cfg.strings.status_cancelled || "Reserva cancelada" : cfg.strings.pending_title || "Tu reserva est\xE1 registrada";
          var statusNote = isConfirmed ? "" : !isCancelled ? cfg.strings.pending_note || "La reserva se confirmar\xE1 una vez que validemos el pago. Te enviaremos un correo." : "";
          ...
          html += "</div>";
          var pendingPayment = !isCancelled && booking.payment_status === "pending" && booking.payment_requires_external_redirect && Array.isArray(booking.available_payment_gateways) && booking.available_payment_gateways.length > 0;
          if (pendingPayment) {
            ...
            if (booking.available_payment_gateways.length === 1) {
              ...
              html += '<button type="button" class="obwp-btn obwp-btn--primary" data-pay-now="' + escapeHtml(gw.key) + '">' + escapeHtml(cfg.strings.pay_now || "Pagar ahora") + " \xB7 " + escapeHtml(gw.label) + "</button>";
            } else {
              html += '<p class="obwp-note">' + escapeHtml(cfg.strings.choose_gateway || "Elige c\xF3mo pagar:") + "</p>";
              booking.available_payment_gateways.forEach(function(gw2) {
                html += '<button type="button" class="obwp-btn obwp-btn--primary" data-pay-now="' + escapeHtml(gw2.key) + '">' + escapeHtml(gw2.label) + "</button> ";
              });
            }
            html += "</div>";
          }
          if (!isCancelled && (booking.cancel_token || booking.reschedule_token)) {
            html += '<div class="obwp-confirmation-actions">';
            if (booking.reschedule_token) {
              html += '<a href="?obwp_reschedule=' + escapeHtml(booking.reschedule_token) + '" class="obwp-btn obwp-btn--secondary">' + escapeHtml(cfg.strings.reschedule || "Reprogramar") + "</a>";
            }
            if (booking.cancel_token) {
              html += '<a href="?obwp_cancel=' + escapeHtml(booking.cancel_token) + '" class="obwp-btn obwp-btn--secondary">' + escapeHtml(cfg.strings.cancel || "Cancelar reserva") + "</a>";
            }
            html += "</div>";
          }
          html += "</div>";
          ...
          $app.off("click.obwpPayNow").on("click.obwpPayNow", "[data-pay-now]", function() {
            ...
            Api.post("payments/create", {
              ...
              if (res2 && res2.checkout && res2.checkout.redirect_url) {
                window.location.href = res2.checkout.redirect_url;
                return;
              }
              $("[data-pay-actions]").html('<p class="obwp-error">' + escapeHtml(res2 && res2.error || cfg.strings.error_generic || "No pudimos iniciar el pago.") + "</p>");
            }).fail(function(xhr) {
              $("[data-pay-actions]").html('<p class="obwp-error">' + escapeHtml(Api.errorMsg(xhr)) + "</p>");
            });
          });
        }).fail(function(xhr) {
          var msg = xhr.status === 404 ? cfg.strings.booking_not_found || "No encontramos esta reserva. El enlace puede haber expirado." : Api.errorMsg(xhr);
          $app.html(
            '<div class="obwp-token-action obwp-state obwp-state--failed" role="alert"><div class="obwp-state-icon" aria-hidden="true">&#9888;</div><h2>' + escapeHtml(msg) + "</h2></div>"
          );
        });
      },
      _startConfirmAttendance: function(token) {
        var $app = $("#obwp-booking-app");
        $app.html('<div class="obwp-token-action"><p class="obwp-loading">' + (cfg.strings.loading || "Cargando...") + "</p></div>");
        Api.get("bookings/confirm-attendance/" + token).done(function(res) {
          if (res.already_confirmed) {
            var b = res.booking || {};
            var dateStr = b.start_at ? b.start_at.replace("T", " ") : "";
            $app.html('<div class="obwp-token-action obwp-state obwp-state--success"><div class="obwp-state-icon" aria-hidden="true">&#10003;</div><h2>' + escapeHtml(cfg.strings.attendance_already_confirmed || "Ya hab\xEDas confirmado tu asistencia.") + "</h2><p>" + escapeHtml(dateStr) + "</p></div>");
            return;
          }
          if (!res.can_confirm_attendance) {
            $app.html('<div class="obwp-token-action obwp-state obwp-state--failed"><div class="obwp-state-icon" aria-hidden="true">&#9888;</div><h2>' + escapeHtml(cfg.strings.attendance_unavailable || "No se puede confirmar asistencia") + "</h2></div>");
            return;
          }
          var b = res.booking || {};
          var dateStr = b.start_at ? b.start_at.replace("T", " ") : "";
          $app.html(
            '<div class="obwp-token-action"><h2>' + escapeHtml(cfg.strings.attendance_title || "Confirmar asistencia") + "</h2><p>" + escapeHtml(b.service_name || "") + "</p><p><strong>" + escapeHtml(dateStr) + '</strong></p><button class="obwp-btn obwp-btn--primary" id="obwp-confirm-attendance-btn">' + escapeHtml(cfg.strings.attendance_confirm_btn || "S\xED, confirmar asistencia") + '</button> <a href="' + escapeHtml(window.location.pathname) + '" class="obwp-btn obwp-btn--secondary">' + escapeHtml(cfg.strings.no || "No, volver") + "</a></div>"
          );
          $("#obwp-confirm-attendance-btn").on("click", function() {
            var $btn = $(this).prop("disabled", true).text(cfg.strings.loading || "Procesando...");
            Api.post("bookings/confirm-attendance/" + token, {}).done(function(postRes) {
              if (postRes.already_confirmed) {
                $app.html('<div class="obwp-token-action obwp-state obwp-state--success"><div class="obwp-state-icon" aria-hidden="true">&#10003;</div><h2>' + escapeHtml(cfg.strings.attendance_already_confirmed || "Ya hab\xEDas confirmado tu asistencia.") + "</h2></div>");
                return;
              }
              if (postRes.success) {
                var postB = postRes.booking || {};
                var postDateStr = postB.start_at ? postB.start_at.replace("T", " ") : "";
                $app.html('<div class="obwp-token-action obwp-state obwp-state--success"><div class="obwp-state-icon" aria-hidden="true">&#10003;</div><h2>' + escapeHtml(cfg.strings.attendance_success || "\xA1Confirmado! Te esperamos el ") + escapeHtml(postDateStr) + ".</h2></div>");
              } else {
                $app.html('<div class="obwp-token-action obwp-state obwp-state--failed"><div class="obwp-state-icon" aria-hidden="true">&#9888;</div><h2>' + escapeHtml(cfg.strings.attendance_failed || "No se pudo confirmar") + '</h2><p class="obwp-error">' + escapeHtml(postRes.error || cfg.strings.error_generic) + "</p></div>");
              }
            }).fail(function(xhr) {
              var msg;
              if (xhr.status === 404) {
                msg = cfg.strings.attendance_invalid || "Este enlace ya no es v\xE1lido.";
              } else if (xhr.status === 410) {
                msg = cfg.strings.attendance_past || "La cita ya ocurri\xF3.";
              } else if (xhr.status === 409) {
                msg = cfg.strings.attendance_not_confirmed || "La reserva a\xFAn no ha sido confirmada.";
              } else {
                msg = Api.errorMsg(xhr);
              }
              $app.html('<div class="obwp-token-action obwp-state obwp-state--failed"><div class="obwp-state-icon" aria-hidden="true">&#9888;</div><h2>' + escapeHtml(cfg.strings.attendance_failed || "No se pudo confirmar") + '</h2><p class="obwp-error">' + escapeHtml(msg) + "</p></div>");
            });
          });
        }).fail(function(xhr) {
          var msg;
    ...
    var Services = {
      ...
      load: function() {
        ...
        this._loadTimer = setTimeout(function() {
          ...
          if (current.length) {
            $list.html(
              '<p class="obwp-loading">' + (cfg.strings.loading || "Cargando servicios...") + '</p><p class="obwp-note">' + escapeHtml(cfg.strings.load_slow || "La carga est\xE1 demorando m\xE1s de lo habitual.") + "</p>"
            );
          }
        }, 8e3);
        Api.get("services").done(function(res) {
          ...
          if (State.serviceSlug) {
            services = services.filter(function(s) {
              return s.slug === State.serviceSlug;
            });
          }
          if (!services.length) {
            $list.html(
              '<p class="obwp-empty">' + (cfg.strings.no_services || "No hay servicios disponibles en este momento.") + '</p><p class="obwp-empty-help">' + escapeHtml(cfg.strings.no_services_help || "Intenta nuevamente m\xE1s tarde o contacta al negocio.") + "</p>"
            );
          ...
          if (State.serviceId) {
            var restored = services.filter(function(s) {
              return String(s.id) === String(State.serviceId);
            })[0] || null;
            if (restored) {
          ...
          services.forEach(function(s) {
            html += '<div class="obwp-service-card" role="button" tabindex="0" data-id="' + s.id + '" data-name="' + escapeHtml(s.name) + '" data-price="' + escapeHtml(s.formatted_price || "") + '" data-price-minor="' + (s.price_minor || 0) + '" data-currency="' + escapeHtml(s.currency || "") + '">';
          ...
          $list.html(
            '<div class="obwp-state obwp-state--failed" role="alert"><p class="obwp-error">' + (cfg.strings.error_services_load || "No pudimos cargar los servicios.") + '</p><button class="obwp-btn obwp-btn--primary" id="obwp-retry-services">' + (cfg.strings.retry || "Volver a intentar") + "</button></div>"
          );
    ...
    var Events = {
      bind: function() {
        ...
        $(document).on("click.obwp", "#obwp-prev-month", function(e) {
          e.preventDefault();
          Calendar.change(-1);
        });
        $(document).on("click.obwp", "#obwp-next-month", function(e) {
          e.preventDefault();
          Calendar.change(1);
        });
        $(document).on("click.obwp keydown.obwp", ".obwp-calendar-day--available", function(e) {
          ...
          if (e.type === "keydown") {
            ...
            if (e.which === 39 && idx < $days.length - 1) {
              e.preventDefault();
              $days.eq(idx + 1).focus();
              return;
            }
            ...
            if (e.which === 40 && idx + 7 < $days.length) {
              e.preventDefault();
              $days.eq(idx + 7).focus();
              return;
            }
        ...
        $(document).on("click.obwp keydown.obwp", ".obwp-time-slot", function(e) {
          ...
          if (e.type === "keydown") {
            ...
            if (e.which === 37 && idx > 0) {
              e.preventDefault();
              $slots.eq(idx - 1).focus();
              return;
            }
            if (e.which === 39 && idx < $slots.length - 1) {
              e.preventDefault();
              $slots.eq(idx + 1).focus();
              return;
            }
    ...
    function init() {
      ...
      if (Session.restore()) {
        FlowNotice.show(cfg.strings.booking_restore || "Recuperamos tu progreso para que contin\xFAes donde lo dejaste.", true);
        Summary.render();
        Baseline.markFirstInteraction("restore", { step: State.step });
      }
      if (TokenFlow.handle()) return;
      Api.get("form-fields-public").done(function(res) {
        State.formFields = res.fields || [];
      }).fail(function() {
        State.formFields = [];
      });
      Services.load();
